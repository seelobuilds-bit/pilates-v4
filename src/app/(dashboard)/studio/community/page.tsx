...
export default function StudioCommunityPage() {
  ...
  async function fetchPlans() {
    try {
      ...
      if (res.ok) {
        ...
        // Auto-select first plan with chat
        const firstWithChat = data.plans?.find((p: SubscriptionPlan) => p.communityChat)
        if (firstWithChat) {
          setSelectedPlan(firstWithChat)
        }
      }
    } catch (err) {
  ...
  return (
    <div className="px-3 py-4 sm:px-4 sm:py-5 lg:p-8 bg-gray-50/50 h-[calc(100dvh-3.5rem)]">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Community</h1>
        <p className="text-gray-500 mt-1">Manage and participate in your subscription community chats</p>
      </div>
        ...
        <div className="space-y-3">
          ...
          {subscriptionPlans.map(plan => (
            <Card 
              ...
              onClick={() => plan.communityChat && setSelectedPlan(plan)}
            >
