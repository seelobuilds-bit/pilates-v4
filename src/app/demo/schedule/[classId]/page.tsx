...
export default function ClassSessionDetailPage({
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch class session
        const classRes = await fetch(`/api/demo/schedule/${resolvedParams.classId}`)
        if (classRes.ok) {
        ...
        // Fetch locations
        const locationRes = await fetch('/api/demo/locations')
        if (locationRes.ok) {
  ...
  useEffect(() => {
    async function fetchRecurringCount() {
      ...
      try {
        const res = await fetch(`/api/demo/schedule?recurringGroupId=${classSession.recurringGroupId}&futureOnly=true`)
        if (res.ok) {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      const res = await fetch(`/api/demo/schedule/${resolvedParams.classId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleSendMessageToAll = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/demo/messages/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          clientIds,
  ...
  return (
    <div className="px-3 py-4 sm:px-4 sm:py-5 lg:p-8 bg-gray-50/50 min-h-screen">
      ...
      {/* Header */}
      <div className="mb-8">
        <Link href="/demo/schedule" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 mb-4">
              ...
              {classSession.bookings && classSession.bookings.length > 0 ? (
                <div className="space-y-2">
                  {classSession.bookings.map((booking) => (
                    <Link key={booking.id} href={`/demo/clients/${booking.id}`}>
