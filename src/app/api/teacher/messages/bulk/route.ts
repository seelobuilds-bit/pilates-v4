      ...
      for (const client of clients) {
        try {
          await transporter.sendMail({
            from: `"${teacherName} - ${studio.name}" <${emailConfig.email.fromEmail || emailConfig.email.smtpUser}>`,
            to: client.email,
            ...
            html: `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <p style="color: #666; font-size: 12px;">From ${teacherName} at ${studio.name}</p>
            </div>`
          })

          // Log message (shared inbox)
