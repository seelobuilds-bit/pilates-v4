...
// POST - Create/Update email config and start domain verification
...
// PATCH - Check domain verification status or trigger verification
...
// DELETE - Remove email config and domain
export async function DELETE() {
  try {
    ...
    // Delete domain from Resend if exists
    if (config.resendDomainId) {
      await deleteDomain(config.resendDomainId)
    }

    // Delete config
    await db.studioEmailConfig.delete({
      where: { studioId }
    })

    return NextResponse.json({ message: "Email config deleted" })
  } catch (error) {
